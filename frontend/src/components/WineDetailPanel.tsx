...
function formatGrapes(p: ProductOut): string | null {
  return p.grape ?? null
}
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
                  ...
                  apiClient<ProductOut>(`/products/${sku}`)
                    ...
                    .catch((err) =>
                      setPanelState({
                        status: 'error',
                        message:
                          err instanceof ApiError ? err.detail : t('wineDetail.failedToLoad'),
                      }),
                    )
                  setLoadedSku(null)
                }}
        ...
        {panelState.status === 'loaded' &&
          ...
          (() => {
            ...
            const dotColor = p.category
              ? (CATEGORY_DOT[p.category] ?? 'bg-muted-foreground/30')
              : null
            const origin = formatOrigin(p)
            const grapes = formatGrapes(p)

            const details: { label: string; value: string }[] = []
            if (origin) details.push({ label: t('wineDetail.region'), value: origin })
            if (p.appellation)
              details.push({ label: t('wineDetail.appellation'), value: p.appellation })
